...
const report={method:external?'User-supplied local raw/text pairs':corpus.selection,
  ...
  notes:'Exact output matching, including variants and spaces. Readings are supplied, not inferred by the tested dictionary. Corpus overlap with upstream frequency training is unknown. CER includes insertions and can exceed 100%. Timing uses an already-loaded WASM module; prefix latency excludes engine construction and UI rendering.',
  corpusSha256:createHash('sha256').update(JSON.stringify(cases)).digest('hex'),profiles:{}};
for(const profile of ['prototype','expanded']) {
  const started=performance.now();const engine=createEngine({dictionary:profile});
  ...
  try {
    ...
    report.profiles[profile]={dictionary:engine.dictionarySize(),instanceCreationMs:initializationMs,groups,prefixLatency:{count:prefixTimes.length,p50Ms:prefixTimes[Math.floor(prefixTimes.length*.5)],p95Ms:prefixTimes[Math.floor(prefixTimes.length*.95)],maxMs:prefixTimes.at(-1)},rows};
...
for(const [profile,result] of Object.entries(report.profiles))lines.push(`${profile}: engine instance ${result.instanceCreationMs.toFixed(1)} ms; prefix decode p50 ${result.prefixLatency.p50Ms.toFixed(2)} ms, p95 ${result.prefixLatency.p95Ms.toFixed(2)} ms (this run; module already loaded).`);
...
if(external){console.log(JSON.stringify(report,null,2));}
else {
