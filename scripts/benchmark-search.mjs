...
const inputs=[...cases,...diversityCases].flatMap(row=>Array.from({length:row.raw.length},(_,i)=>({raw:row.raw.slice(0,i+1),options:row.options})));
const measure=name=>{
  const times=[];
...
const runs=[];
try{
  const recorded=JSON.parse(await readFile(new URL('../eval/diversity-baseline.json',import.meta.url),'utf8'));
  for(const row of recorded.rows){
    ...
    assert.deepEqual(normalize(candidates),normalize(row.runs[0].candidates),`Baseline snapshot mismatch: ${row.id}`);
  }
